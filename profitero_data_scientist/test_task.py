...
use_case = UseCases.UC5
country = Country.US
category = Category.SPORT___ENERGY_DRINKS
paths = Paths(country=country, category=category, use_case=use_case)
data = Data(paths)
train_set = data.get_data_frame_from_csv(DataSetType.TRAIN_TRAIN)
test_set = data.get_data_frame_from_csv(DataSetType.TRAIN_TEST)
...
all_predictors = data.cols_in_train_set.union(data.cols_in_test_set) - data.output_cols_from_metadata
...
# responses = ['global_outer_packaging']
...
def get_best_predictors(current_set_of_predictors, current_best_predictors, response):
    ...
    for predictor in current_set_of_predictors:
        ...
        try:
            ...
            print ('error occured, the number of class labels must be greater than one, was working on', predictor, response, ValueError.message)
            break
    ...
    print ('best keys:', keys_with_max_val(tmp_dict_1))
...
def keys_with_max_val(dct):
    return [(k, v) for k, v in dct.iteritems() if v == max(dct.itervalues())]


def map_to_json(map):
    json_str = '{'
    for output in map:
        json_str = json_str + '"' + output + '": [[['
        inputs = map.get(output).keys()
        json_str = json_str + '"' + '", "'.join(inputs[0]) + '"'
        json_str = json_str + '], ' + str(map.get(output).get(inputs[0])) + ']],'
