...
class ChModel:
    ...
    def prepare(self, X):
        X_vectorized = Vectorizer(Vectorizers.ch_revi_1_5_ngram_count).transform(X[Field.review])
        # tmp = Vectorizer(Vectorizers.ch_prod_1_3_ngram_count).transform(X[Field.product_name])
        ...
        # X_vectorized = hstack((X_vectorized, np.array(X_train[Field.product_category])[:, None]))
    ...
    def train(self, train_file=None):
        ...
        train = Data.Data(train_file).get_original_df()
        train.dropna(inplace=True)

        X_train = train[[Field.review, Field.created_at, Field.product_name]]
        Y_train = train[[Field.stars]]

        X_train, X_train_vectorized = self.prepare(X_train)
        print('X_train:', X_train.shape, 'Y_train:', Y_train.shape)
        print('X_train_vectorized:', X_train_vectorized.shape)

        self.model.train(X_train_vectorized, Y_train.values.ravel())

    def extract(self, test_file):
        print('extracting from model')

        test = Data.Data(test_file).get_original_df()
        ...
        X_test, X_test_vectorized = self.prepare(X_test)
        print('X_test:', X_test.shape, 'Y_test:', Y_test.shape)
        print('X_test_vectorized', X_test_vectorized.shape)

        self.model.extract(X_test_vectorized, Y_test)

        output = pd.DataFrame({Field.review_id: test[Field.review_id],
                               Field.stars: self.model.predictions
                               })
...
model = ChModel(Models.model_ch_1)
# model = ChModel(Models.model_ch_2) # the model is huge and was not attached
# model.train()
model.extract(File.test)
